...
export class GroqLlmClient {
    ...
    async mapFields(fields, catalog) {
        const userPayload = {
            fields: fields.map((f) => ({
                refId: f.refId,
                label: f.label,
 ...
 * GroqLlmClient.
 */
export class MockSemanticLlmClient {
    name = "mock-offline";
    static SYNONYMS = {
    ...
    async mapFields(fields, catalog) {
        ...
        const tokensOf = (s) => new Set(expand(normLabel(s)).split(" ").filter((t) => t.length > 2));
        ...
        const weight = (t) => 1 / (1 + Math.log(1 + (docFreq.get(t) || 1)));
        return fields.map((f) => {
