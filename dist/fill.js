...
const refLocator = (page, refId) => page.locator(`[data-agent-ref="${refId}"]`).first();
export async function applyMapping(page, field, mapping, answers) {
    ...
    if (mapping.answerKey === "__TODAY__") {
        ...
        if (mapping.answerKey.includes(".")) {
            ...
            const row = field.repeat?.rowIndex ?? 0;
            const arr = answers[arrayKey]?.answer;
            ...
            raw =
                answers[mapping.answerKey]?.answer;
        }
    }
    switch (mapping.action) {
        case "fill": {
            ...
            if (field.kind === "date") {
                ...
                if (actualDate !== value) {
                    throw new Error(`Failed filling "${field.label}" expected "${value}" got "${actualDate}"`);
                }
                break;
            }
            await locator.click();
            const modifier = globalThis.process?.platform === "darwin"
                ? "Meta"
                : "Control";
            ...
            if (normalize(actual) !== normalize(value)) {
                ...
                if (normalize(actual) !== normalize(value)) {
                    throw new Error(`Failed filling "${field.label}" expected "${value}" got "${actual}"`);
                }
            }
            break;
        }
        case "select": {
            ...
            if (selected !== mapping.chosenOptionValue) {
                throw new Error(`${field.label} select failed`);
            }
            break;
        }
        case "check": {
            ...
            if (field.kind === "radio-group") {
                const radio = page.locator(`input[type="radio"][data-agent-ref="${field.refId}"]`);
                ...
                const target = (mapping.chosenOptionValue ?? String(raw)).toLowerCase();
                for (let i = 0; i < count; i++) {
                    const value = (await radio.nth(i).getAttribute("value")) ?? "";
                    if (value.toLowerCase() === target) {
                        await radio.nth(i).check();
                        return;
                    }
                }
                throw new Error(`No radio option matched "${mapping.chosenOptionValue ?? raw}"`);
            }
            ...
            if (!(await locator.isChecked())) {
                throw new Error(`${field.label} checkbox failed`);
            }
            break;
        }
        case "uncheck": {
 ...
 */
export function matchArrayAnswerKey(group, catalog) {
 ...
 */
export async function mapRepeatRowTemplate(templateFields, itemKeys, llm) {
    const pseudoCatalog = itemKeys.map((k) => ({ key: k, isArray: false, preview: k }));
    const results = [];
    const needsLlm = [];
    const manualMap = {
        "code": "code",
    ...
    templateFields.forEach((f, idx) => {
        ...
        for (const k of itemKeys) {
            const score = label.includes(normLabel(k))
                ? 1
                : 0;
            if (!best || score > best.score) {
                best = { key: k, score };
    ...
    if (needsLlm.length > 0) {
        const raw = await llm.mapFields(needsLlm.map((n) => n.field), pseudoCatalog);
        for (const r of raw) {
            const hit = needsLlm.find((n) => n.field.refId === r.refId);
            if (hit && r.answerKey)
                results[hit.idx] = r.answerKey;
        }
    }
    return results;
}
