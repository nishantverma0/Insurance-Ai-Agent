...
async function extractInBrowser() {
  ...
  function isVisible(el: Element): boolean {
    ...
    if (rect.right <= 0 || rect.bottom <= 0 || rect.left >= docW || rect.top >= docH) return false;
    ...
    while (node) {
      ...
      if ((node as HTMLElement).getAttribute && (node as HTMLElement).getAttribute("aria-hidden") === "true") return false;
  ...
  function findHelperText(el: HTMLElement, label: string): string {
    const parts: string[] = [];
    ...
    if (container) {
      ...
      smallEls.forEach((s) => {
        const t = textOf(s);
        if (t && t.length < 120 && t !== label) parts.push(t);
      });
    }
  ...
  function labelSuggestsRequired(label: string): boolean {
  ...
  function looksLikeAttestation(label: string): boolean {
    const t = label.toLowerCase();
    return /\bcertif(y|ies|ication)\b/.test(t) || /\backnowledge(s)?\b/.test(t) || /\bagree(s)?\s+that\b/.test(t);
  }

  function kindFor(el: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement): string {
    if (el.tagName === "TEXTAREA") return "textarea";
    if (el.tagName === "SELECT") return "select";
    const t = (el as HTMLInputElement).type;
    if (t === "radio") return "radio-group";
    if (t === "checkbox") return "checkbox";
    if (["email", "tel", "date", "number"].includes(t)) return t;
    return "text";
  }

  // --- discover repeating row groups -----------------------------------
  // Heuristic: a container whose children are >=1 structurally-identical,
  // *currently rendered* sibling blocks (same multiset of input types),
  // sitting next to a control whose visible text suggests "add another".
  type RepeatInfo = { groupEl: Element; addBtn: Element | null; rows: Element[] };
  const repeatGroups: RepeatInfo[] = [];
  const candidateContainers = new Set<Element>();
...
.filter(c =>
    ...
    const addBtn =
      (Array.from(container.querySelectorAll('button,a')) as Element[]).find((b) =>
        isVisible(b) && /\b(add|new|another|\+)\b/i.test(textOf(b))
      ) ?? null;
    ...
    const sig = (c: Element) =>
      Array.from(c.querySelectorAll("input, select, textarea"))
        .map((n) => (n as HTMLInputElement).type || n.tagName)
        .sort()
        .join(",");
...
sigs.every(s=>s===sigs[0]);
  ...
  const rowMembership = new Map<Element, { groupIdx: number; rowIndex: number }>();
  ...
  function ownerRow(el: Element): { groupIdx: number; rowIndex: number } | null {
    let node: Element | null = el;
    while (node) {
  ...
  allInputs.forEach((el) => {
    if ((el as HTMLInputElement).type === "radio") {
      ...
      const first = group[0] as HTMLInputElement;
      ...
      const options = group.map((g) => ({
        ...
        text: findLabelFor(g as HTMLInputElement) || (g as HTMLInputElement).value,
      }));
      const checked = group.find((g) => (g as HTMLInputElement).checked) as HTMLInputElement | undefined;
      const ref = nextRef();
      ...
      fields.push({
        ...
        kind: "radio-group",
        label: groupLabel,
        required: group.some((g) => (g as HTMLInputElement).required) || labelSuggestsRequired(groupLabel),
        visible,
    ...
    const label = findLabelFor(el as HTMLElement);
    const kind = kindFor(el);
    const visible = isVisible(el);
    const helper = findHelperText(el as HTMLElement, label);
    ...
    if (kind === "checkbox") currentValue = String((el as HTMLInputElement).checked);
    else currentValue = (el as HTMLInputElement | HTMLTextAreaElement).value;
    ...
    (el as HTMLElement).classList.contains("required") ||
...
const actionButtons: { refId: string; text: string }[] = [];
...
if(
  ...
  const ref = nextRef();
  (el as HTMLElement).setAttribute("data-agent-ref", ref);
...
document
  .querySelectorAll(
    '[role="alert"],[class*="err" i],[class*="invalid" i],[class*="validation" i],[class*="warning" i],.error,.errors,.validation-summary-errors,.field-validation-error,span.error,li.error'
  )
  .forEach((e) => {
    if (isVisible(e)) {
      const t = textOf(e);
      if (t) visibleErrors.push(t);
    }
  });
...
export async function perceivePage(page: Page, pageIndex: number): Promise<PagePerception> {
