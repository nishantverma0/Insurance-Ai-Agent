...
import type { AnswerMap, DecisionLogEntry, FlaggedField, PagePerception, RunReport } from "./types.js";
...
function fieldIdentity(f: { label: string; kind: string }): string {
...
export async function runAgent({ formPath, answers, llm, headless = true }: AgentOptions): Promise<RunReport> {
  const startedAt = new Date().toISOString();
  const decisionLog: DecisionLogEntry[] = [];
  const fieldsFlagged: FlaggedField[] = [];
  ...
  let outcome: RunReport["outcome"] = "error";
  let reason: string | undefined;
  let confirmationData: unknown | null = null;
  let pagesVisited = 0;
  let fieldsFilled = 0;

  const log = (entry: Omit<DecisionLogEntry, "timestamp">) => decisionLog.push({ timestamp: new Date().toISOString(), ...entry });

  try {
    for (let pageIndex = 0; pageIndex < MAX_PAGES; pageIndex++) {
      pagesVisited++;
      let perception: PagePerception = await perceivePage(page, pageIndex);
      ...
      for (const group of perception.repeatGroups) {
        ...
        const items = matchedEntry.arrayItemKeys ? ((answers[matchedEntry.key].answer as Record<string, unknown>[]) ?? []) : [];
        ...
        const liveGroup = perception.repeatGroups.find(
          g => JSON.stringify(g.rowFieldLabels) === JSON.stringify(group.rowFieldLabels)
        );
        ...
        const rowGroups: Record<number, typeof perception.fields> = {};

        perception.fields
          .filter(f =>
            f.repeat &&
            JSON.stringify(
              perception.repeatGroups.find(g => g.groupRefId === f.repeat!.groupRefId)?.rowFieldLabels
            ) === JSON.stringify(group.rowFieldLabels)
          )
          .forEach(f => {
        ...
        for (let rowIdx = 0; rowIdx < items.length; rowIdx++) {
          ...
          rowFields.sort(
            (a, b) =>
              templateFields.findIndex(t => t.label === a.label) -
      ...
      for (let rescan = 0; rescan < MAX_CONDITIONAL_RESCANS; rescan++) {
        ...
        const candidates = perception.fields.filter(
          f => f.visible && !f.repeat && !processed.has(fieldIdentity(f))
        );
        ...
        const subPerception: PagePerception = {
          ...perception,
          fields: candidates
        };
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS_PER_PAGE; attempt++) {
        ...
        if (check.success) {
          outcome = "success";
          confirmationData = (check as any).confirmationData ?? null;
          break;
        }
